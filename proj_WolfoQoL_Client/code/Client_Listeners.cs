...
namespace WolfoQoL_Client
{
    public static class ClientChecks
    {
        public static void Start()
        {
            ...
            On.EntityStates.DroneScrapper.DroneScrapperBaseState.OnEnter += ScrappingDroneMessage_Client;
        ...
        private static void ScrappingDroneMessage_Client(On.EntityStates.DroneScrapper.DroneScrapperBaseState.orig_OnEnter orig, EntityStates.DroneScrapper.DroneScrapperBaseState self)
        {
            orig(self);
            if (!NetworkServer.active)
            {
                if (self is EntityStates.DroneScrapper.DroneScrapping)
                {
        ...
        private static void Inventory_OnDeserialize(ILContext il)
        {
            ILCursor c = new ILCursor(il);
            if (c.TryGotoNext(MoveType.Before,
             x => x.MatchLdflda("RoR2.Inventory", "permanentItemStacks")))
            //x => x.MatchCall("RoR2.NetworkExtensions", "ReadItemStacks")))
            {
                c.EmitDelegate<Func<Inventory, Inventory>>((inventory) =>
                {
                    ...
                    if (inventory.TryGetComponent<PlayerItemLoss_ClientListener>(out a))
                    {
                        ...
                        inventory.WriteAllPermanentItemStacks(a.prevItems);
        ...
        private void Master_onBodyStart(CharacterBody obj)
        {
            ...
            if (obj.isRemoteOp)
            {
                ...
                Chat.AddMessage(new DroneChatMessage
                {
                    ...
                    subjectAsNetworkUser = playerMaster.playerCharacterMasterController.networkUser,
                });
            }
        }
    ...
    public class DevotionItemLoss_ClientListener : ItemLossBase_ClientListener
    {
        ...
        public override void Inventory_onInventoryChanged()
        {
            ...
            SingleItemLoss(inventory.permanentItemStacks); //Try every time I guess??
        }
        ...
        public bool SingleItemLoss(ItemCollection now)
        {
            ...
            {
                if (prevItems[i] > now.GetStackValue((ItemIndex)i))
                {
                    send = true;
                    item = i;
                    itemCount = prevItems[i] - now.GetStackValue((ItemIndex)i);
                    break;
                }
            }
            if (send)
            ...
            inventory.WriteAllPermanentItemStacks(prevItems);
    ...
    public class DroneLoss_ClientListener : MonoBehaviour
    {
        ...
        public void OnDisable()
        {
            lastDroneOwner = this.GetComponent<MinionOwnership>().ownerMaster.playerCharacterMasterController.networkUser;
            lastDeadDrone = this.GetComponent<CharacterMaster>().bodyPrefab;
            tier = this.GetComponent<Inventory>().GetItemCountPermanent(DLC3Content.Items.DroneUpgradeHidden);
        }

        public static void SendMessage()
        {
            if (lastDroneOwner == null)
            {
                return;
            }
            if (lastDeadDrone == null)
            {
                return;
            }
            Chat.AddMessage(new ItemLossMessage
            {
                ...
                subjectAsNetworkUser = lastDroneOwner,
            });
        }
    }
    ...
    public class PlayerItemLoss_ClientListener : ItemLossBase_ClientListener
    {
        ...
        public static void TrySaleStar(PurchaseInteraction purchase)
        {
            ...
            if (latest != null)
            {
                ...
                if (salePre > 0 && salePost == 0)
                {
                    ...
                    Chat.AddMessage(new InteractableMessage
                    {
        ...
        public override void TryMessageInstance()
        {
            ...
            {
                succeeded = ItemScrapMessage(inventory.permanentItemStacks);
            }
            else if (forceEquip)
            {
            ...
            {
                succeeded = ItemMessage(inventory.permanentItemStacks);
            }
            if (!succeeded)
            {
        ...
        public bool ItemMessage(ItemCollection now)
        {
            ...
            for (int i = 0; i < prevItems.Length; i++)
            {
                if (prevItems[i] > now.GetStackValue((ItemIndex)i))
                {
                    itemsTaken++;
                    item = i;
                    itemLosses[i] = prevItems[i] - now.GetStackValue((ItemIndex)i);
                }
            }
            if (itemsTaken == 1)
            {
